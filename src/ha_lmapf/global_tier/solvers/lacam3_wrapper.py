...
class LaCAM3Solver(BaseSolverWrapper):
    """
    ...
    def plan_with_metadata(
        ...
        """SolverResult-returning entry point for LaCAM3.

        Builds the instance files + command, then routes the
        subprocess invocation through
        :meth:`BaseSolverWrapper._wrap_subprocess` which owns timing
        and status discrimination.  Anytime: when the binary is killed
        by the watchdog after the budget but has already written a
        partial ``result.txt``, the parser surfaces that plan and the
        decision tree classifies it as ``partial_anytime``.
        """
        ...
        try:
            ...
            def parse_fn(stdout: str, stderr: str, returncode: int):
                paths = None
                err = None
                # Read result file once: LaCAM3 writes both the
                # ``solved=0|1`` status field and ``comp_time=<ms>``
                # in the same file alongside any solution.
                content = ""
                ...
                # LaCAM3 self-reports budget exhaustion via ``solved=0``
                ...
                # LaCAM3 emits ``comp_time=<ms>`` in the result file.
