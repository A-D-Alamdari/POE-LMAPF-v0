...
| # | Issue | Source | Effect on paper |
|---|---|---|---|
| 2.1 | **`WaitBasedResolver` and `TokenBasedResolver` use the same `(-d, w, -id)` priority tuple.**  TokenBased's only differentiator is per-cell single-owner + K-rotation; the paper claims `(τ, -d, w)` with a per-(agent, cell) token count.  Code does not implement the count term at all. | `03_local_tier.md` §2.token | The WaitBased vs TokenBased comparison in the paper may be **vacuous**: stripped of WaitBased's `β·𝟙[w>w*]` starvation boost, the two resolvers share the same priority function and differ only in the K-rotation overlay (see audit `03_local_tier.md` §2).  Any §5.5 "WaitBased vs TokenBased" delta therefore measures the rotation policy, not two distinct priority mechanisms.  The paper's `τ` (per-(agent, cell) token count) is absent from the code path entirely |
...
| 2.3 | **Externals are vertex-coordinated.**  Humans block on agent-occupied cells via `agent_positions` passed to every model's `step()`.  $N_x$ measures buffer overlaps, not vertex collisions, because vertex collisions cannot occur. | `04_global_humans.md` §3.3 | The paper's "uncoordinated externals" claim is **materially incomplete**: coordination is suppressed only at the buffer / safety-radius level, while vertex coordination is structural (audit `04_global_humans.md` §3.3 reproduced 0/500 forced-forward entries into an agent cell).  Direct experimental implication: **$N_x$ at `r_safe = 1` only counts the single Manhattan-distance-1 ring**, because distance 0 is structurally precluded by the simulator's cell-blocking of humans, and the paper does not disclose this.  Affects interpretation of $N_x$ as a "human–agent collision" proxy across every §5.3 / §5.4 row |
...
| 3.3 | **6 permanently deselected tests across the audit series.**  The 6 in `test_full_migration_manifest.py::test_wrapper_round_trip_consistency` are an `lacam3` wall-time flake (passes ~1 in 3 in isolation on the audit host).  The earlier `test_paper_claims.py` failures (3-tuple unpacking from commit `812fc90`) were fixed in the resume branch — tests now unpack `(verdicts, structural_claims, validity_report)` and assert the synth fixtures clear the degenerate-run guard. | `05_io_runners.md` summary; subsequent audits' `--deselect` lines | Consider widening the lacam3 timing tolerance |
