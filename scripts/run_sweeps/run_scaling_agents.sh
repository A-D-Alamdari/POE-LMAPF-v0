#!/bin/bash
# Sweep launcher for configs/eval/paper/scaling_agents.yaml
#
# Usage (run on your local machine in tmux):
#     tmux new -d -s sweep_scaling_agents bash scripts/run_sweeps/run_scaling_agents.sh
#
# Monitor progress:
#     watch -n 10 'wc -l logs/paper/scaling_agents/results.csv; \
#                  cat logs/paper/scaling_agents/.heartbeat'
#
# Attach to the tmux session to watch live output:
#     tmux attach -t sweep_scaling_agents
#
# Resume after interruption (kill, machine reboot, etc.):
#     bash scripts/run_sweeps/run_scaling_agents.sh
# --resume in run_paper_experiment.py skips runs already in results.csv.

set -euo pipefail

SWEEP_NAME="scaling_agents"
