#!/bin/bash
# Sweep launcher for configs/eval/paper/budget_sensitivity.yaml
# (E17 — budget-sensitivity defensive sweep, defends §5.1's 10s default).
#
# Usage (run on your local machine in tmux):
#     tmux new -d -s sweep_budget_sensitivity bash scripts/run_sweeps/run_budget_sensitivity.sh
#
# Monitor progress:
#     watch -n 10 'wc -l logs/paper/budget_sensitivity/results.csv; \
#                  cat logs/paper/budget_sensitivity/.heartbeat'
#
# Attach to the tmux session to watch live output:
#     tmux attach -t sweep_budget_sensitivity
#
# Resume after interruption (kill, machine reboot, etc.):
#     bash scripts/run_sweeps/run_budget_sensitivity.sh
# --resume in run_paper_experiment.py skips runs already in results.csv.

set -euo pipefail

SWEEP_NAME="budget_sensitivity"
...
# Long-running call (hours; some cells run at 30s budget so per-run
# wall-clock is higher than the §5.5 baseline).  No bash-level timeout;
# user controls termination via tmux / Ctrl-C.  --resume on re-launch
