# !/usr/bin/env python3
"""Step 1 (faster variant) — Tune the global planning *horizon*.

Small-maps-only version of tune_horizon_fast.py for even faster iteration:

  - Horizon sweep: [20, 30, 40, 50, 60, 70, 80]  (removed 10 and 100)
  - Seeds: 10  (down from 15)
  - time_budget_ms: 3000  (down from 5000)
  - Only 2 small maps (random, warehouse_small): all 6 solvers
  - No large maps

Total runs:
  6 solvers × 2 maps × 4 agent counts × 7 horizons × 10 seeds = 3,360

Settings otherwise match compare_global_study_agents.py:
  - 2 maps (random, warehouse_small)
  - agents=[25,50,75,100], humans=50
  - 1500 steps, fov=4, safety_radius=1
...
Usage:
    # Full sweep (all solvers, small maps only)
    python scripts/tuning/tune_horizon_faster.py --workers 0

    # Quick sanity check
    python scripts/tuning/tune_horizon_faster.py --sanity --workers 0

    # Specific solvers only
    python scripts/tuning/tune_horizon_faster.py --solver lacam lacam3 lns2 --workers 0
"""
...
def _write_cross_solver_latex(
    ...
    lines = [
        ...
        r"averaged across 2~small maps and all agent counts. "
        r"Coupled: \texttt{replan\_every} = $\lfloor H/2 \rfloor$. "
        r"10~seeds, 2000~steps, budget=3s.}",
        r"\label{tab:horizon_per_solver_faster}",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Tune horizon (Step 1, faster variant) — "
            "small maps only, all solvers"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Maps: random, warehouse_small (small maps only, all 6 solvers)\n"
            "\n"
            "Settings match compare_global_study_agents.py:\n"
            "  agents=[25,50,75,100]  humans=50\n"
            "  Seeds=10, steps=1500, fov=4, safety_radius=1, budget=3s\n"
            "\n"
            "Total runs: 3,360\n"
            "\n"
            "Output: logs/tuning/horizon_faster/<label>_<timestamp>/\n"
    ...
    parser.add_argument(
        ...
        help="Simulation steps (default: 1500)",
    )
    parser.add_argument(
        "--maps", type=str, nargs="+",
    ...
    out = make_output_dir("horizon_faster", run_label=args.label, base_dir=args.output)

    # Header
    print(f"\nHorizon Tuning Study (Faster — small maps only)")
    ...
    for mt in map_tags:
        ...
        print(f"    {mt:<20s} agents={mdef['agent_counts']}  "
              f"humans={mdef['num_humans']}  solvers={list(solvers.keys())}")
