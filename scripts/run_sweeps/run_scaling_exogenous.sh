#!/bin/bash
# Sweep launcher for configs/eval/paper/scaling_exogenous.yaml
#
# Usage (run on your local machine in tmux):
#     tmux new -d -s sweep_scaling_exogenous bash scripts/run_sweeps/run_scaling_exogenous.sh
#
# Monitor progress:
#     watch -n 10 'wc -l logs/paper/scaling_exogenous/results.csv; \
#                  cat logs/paper/scaling_exogenous/.heartbeat'
#
# Attach to the tmux session to watch live output:
#     tmux attach -t sweep_scaling_exogenous
#
# Resume after interruption (kill, machine reboot, etc.):
#     bash scripts/run_sweeps/run_scaling_exogenous.sh
# --resume in run_paper_experiment.py skips runs already in results.csv.

set -euo pipefail

SWEEP_NAME="scaling_exogenous"
