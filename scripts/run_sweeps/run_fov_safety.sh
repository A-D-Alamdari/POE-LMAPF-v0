#!/bin/bash
# Sweep launcher for configs/eval/paper/fov_safety.yaml
#
# Usage (run on your local machine in tmux):
#     tmux new -d -s sweep_fov_safety bash scripts/run_sweeps/run_fov_safety.sh
#
# Monitor progress:
#     watch -n 10 'wc -l logs/paper/fov_safety/results.csv; \
#                  cat logs/paper/fov_safety/.heartbeat'
#
# Attach to the tmux session to watch live output:
#     tmux attach -t sweep_fov_safety
#
# Resume after interruption (kill, machine reboot, etc.):
#     bash scripts/run_sweeps/run_fov_safety.sh
# --resume in run_paper_experiment.py skips runs already in results.csv.

set -euo pipefail

SWEEP_NAME="fov_safety"
