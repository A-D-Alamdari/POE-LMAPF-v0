...
Output: ``configs/tuning/allocator_comparison_fov4_safe2.yaml`` — a 200-cell
...
``FOV_RADIUS`` and ``SAFETY_RADIUS`` are HARDCODED at (4, 2) —
one of two operating points evaluated; see the companion
``generate_allocator_comparison_fov3_safe1_yaml.py`` for the
(3, 1) point.  Operating points match the §5.4 scaling sweeps.

Usage
-----

    python scripts/tuning/generate_allocator_comparison_fov4_safe2_yaml.py
"""
...
def _header() -> str:
    return (
        "# Auto-generated by scripts/tuning/generate_allocator_comparison_fov4_safe2_yaml.py.\n"
        ...
        "  replan_every=20; fov_radius=4, safety_radius=2.\n"
        "\n"
        "base:\n"
        "  mode: lifelong\n"
        "  steps: 2000\n"
