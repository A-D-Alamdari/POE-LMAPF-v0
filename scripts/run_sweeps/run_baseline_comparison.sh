#!/bin/bash
# Sweep launcher for configs/eval/paper/baseline_comparison.yaml
#
# Usage (run on your local machine in tmux):
#     tmux new -d -s sweep_baseline_comparison bash scripts/run_sweeps/run_baseline_comparison.sh
#
# Monitor progress:
#     watch -n 10 'wc -l logs/paper/baseline_comparison/results.csv; \
#                  cat logs/paper/baseline_comparison/.heartbeat'
#
# Attach to the tmux session to watch live output:
#     tmux attach -t sweep_baseline_comparison
#
# Resume after interruption (kill, machine reboot, etc.):
#     bash scripts/run_sweeps/run_baseline_comparison.sh
