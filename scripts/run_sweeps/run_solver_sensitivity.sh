#!/bin/bash
# Sweep launcher for configs/eval/paper/solver_sensitivity.yaml
#
# Usage (run on your local machine in tmux):
#     tmux new -d -s sweep_solver_sensitivity bash scripts/run_sweeps/run_solver_sensitivity.sh
#
# Monitor progress:
#     watch -n 10 'wc -l logs/paper/solver_sensitivity/results.csv; \
#                  cat logs/paper/solver_sensitivity/.heartbeat'
#
# Attach to the tmux session to watch live output:
#     tmux attach -t sweep_solver_sensitivity
#
# Resume after interruption (kill, machine reboot, etc.):
#     bash scripts/run_sweeps/run_solver_sensitivity.sh
# --resume in run_paper_experiment.py skips runs already in results.csv.

set -euo pipefail

SWEEP_NAME="solver_sensitivity"
