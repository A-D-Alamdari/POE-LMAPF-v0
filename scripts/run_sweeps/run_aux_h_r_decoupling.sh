#!/bin/bash
# Sweep launcher for configs/eval/paper/aux_h_r_decoupling.yaml
#
# Usage (run on your local machine in tmux):
#     tmux new -d -s sweep_aux_h_r_decoupling bash scripts/run_sweeps/run_aux_h_r_decoupling.sh
#
# Monitor progress:
#     watch -n 10 'wc -l logs/paper/aux_h_r_decoupling/results.csv; \
#                  cat logs/paper/aux_h_r_decoupling/.heartbeat'
#
# Attach to the tmux session to watch live output:
#     tmux attach -t sweep_aux_h_r_decoupling
#
# Resume after interruption (kill, machine reboot, etc.):
#     bash scripts/run_sweeps/run_aux_h_r_decoupling.sh
# --resume in run_paper_experiment.py skips runs already in results.csv.

set -euo pipefail

SWEEP_NAME="aux_h_r_decoupling"
