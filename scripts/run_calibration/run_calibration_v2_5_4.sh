#!/bin/bash
# Calibration cohort: v2 §5.4 — raw_measurements_v2.csv
#
# Per-map num_humans matching configs/eval/paper/scaling_agents.yaml:
#   random-64-64-10:        |X|=20
#   warehouse-10-20-10-2-1: |X|=40
#   warehouse-10-20-10-2-2: |X|=60
# Seeds {0,1,2}; default num_agents per map (4 values each).
# 6 solvers × (4+4+4) cells × 3 seeds × 3 replans = 648 rows.

set -euo pipefail

COHORT="calibration_v2_5_4"
OUT_DIR="logs/calibration"
CSV_NAME="raw_measurements_v2.csv"
...
if [ "${1:-}" != "--keep-existing" ] && [ -f "${CSV_PATH}" ]; then
    if grep -q "conflict_aware_v2_5_4" "${CSV_PATH}" 2>/dev/null; then
...
python scripts/calibrate_solver_budgets.py \
    --out "${OUT_DIR}" \
    --csv-name "${CSV_NAME}" \
    --source-config "conflict_aware_v2_5_4" \
    --num-humans-per-map '{"random-64-64-10":20,"warehouse-10-20-10-2-1":40,"warehouse-10-20-10-2-2":60}' \
    --seeds "0,1,2" \
    --workers "${N_CORES}" \
    --resume \
    2>&1 | tee -a "${LOG_FILE}"
