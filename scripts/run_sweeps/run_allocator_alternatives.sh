#!/bin/bash
# Sweep launcher for configs/eval/paper/allocator_alternatives.yaml
# (E14 — allocator-alternatives defensive sweep, defends §5.4 attribution).
#
# Usage (run on your local machine in tmux):
#     tmux new -d -s sweep_allocator_alternatives bash scripts/run_sweeps/run_allocator_alternatives.sh
#
# Monitor progress:
#     watch -n 10 'wc -l logs/paper/allocator_alternatives/results.csv; \
#                  cat logs/paper/allocator_alternatives/.heartbeat'
#
# Attach to the tmux session to watch live output:
#     tmux attach -t sweep_allocator_alternatives
#
# Resume after interruption (kill, machine reboot, etc.):
#     bash scripts/run_sweeps/run_allocator_alternatives.sh
# --resume in run_paper_experiment.py skips runs already in results.csv.

set -euo pipefail

SWEEP_NAME="allocator_alternatives"
...
# Long-running call (hours).  No bash-level timeout; user controls
