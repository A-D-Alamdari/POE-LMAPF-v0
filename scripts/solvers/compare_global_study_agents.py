#!/usr/bin/env python3
"""
Global Planner Comparison Study for HA-LMAPF.

Compares six global planners across 4 maps and 3 human-model settings
using 10 seeds per configuration. The study measures how solver choice
interacts with map topology and human behavior. Small maps use all 6
solvers; large maps use 4 scalable solvers only.
...
Study matrix
------------
  2 small maps × 3 human models × 6 solvers × variable agent counts × 10 seeds
  2 large maps × 3 human models × 4 solvers × 8 agent counts × 10 seeds

Agent / human scaling per map
-----------------------------
  max_agents = floor_to_lower_100(0.05 × free_cells)
  Small maps — agent counts (tiered by max_agents):
    <= 100: [15, 30, 45, 60, 75, 100]
    <= 200: [25, 50, 75, 100, 125, 150, 175, 200]
    <= 400: [50, 100, 150, 200, 250, 300, 350, 400]
  Large maps — fixed agent counts:
    [100, 250, 400, 550, 700, 850, 1000, 1150]
  num_humans = floor_to_lower_10(0.005 × free_cells)
...
-----
  ...
  # Custom seeds
  python scripts/solvers/compare_global_study_agents.py --seeds 0 1 2
...
def _t_critical(n: int) -> float:
    ...
    # Fallback: pick closest lower df, or use 1.96 for large n
    if df >= 100:
        return 1.960
...
def floor_to_lower_100(x: float) -> int:
    ...
    Examples: 1182 -> 1100, 1100 -> 1100, 99 -> 0
    """
    return int(x // 100) * 100


def floor_to_lower_10(x: float) -> int:
    """Round down to nearest multiple of 10.

    Examples: 118 -> 110, 110 -> 110, 9 -> 0
    """
    return int(x // 10) * 10
...
# Per-map-size solver sets
SMALL_MAP_SOLVER_LABELS: List[str] = [
    "lacam", "lacam3", "pibt2", "lns2", "PBS", "cbsh2-rtc",
]
LARGE_MAP_SOLVER_LABELS: List[str] = [
    "lacam", "lacam3", "pibt2", "lns2",
]

# Maps classified as "large" (use restricted solver set + fixed agent counts)
LARGE_MAPS: set = {"warehouse_large"}

# Fixed agent counts for large maps
LARGE_MAP_AGENT_COUNTS: List[int] = [100, 250, 400, 550, 700, 850, 1000, 1150]
...
# ---------------------------------------------------------------------------
# Map analysis: compute agent counts and human counts
# ---------------------------------------------------------------------------

def analyse_map(map_tag: str, map_path: str, safety_radius: int = 2) -> Dict[str, Any]:
    """Compute free cells, max agents, human count, and agent count list."""
    free_cells = count_free_cells(map_path)
    max_agents = floor_to_lower_100(0.05 * free_cells)
    num_humans = floor_to_lower_10(0.005 * free_cells)

    is_large = map_tag in LARGE_MAPS

    if is_large:
        # Large maps: fixed agent counts
        agent_counts = [c for c in LARGE_MAP_AGENT_COUNTS if c <= max_agents]
        if not agent_counts:
            agent_counts = LARGE_MAP_AGENT_COUNTS  # use full list even if max_agents is small
    elif max_agents < 15:
        # Graceful fallback: use the largest valid positive bucket
        # that fits in free_cells, or skip if none available.
        if free_cells >= 1:
            agent_counts = [min(free_cells - 1, 10)]  # at least 1 free cell for humans
            print(
                f"  WARNING [{map_tag}]: free_cells={free_cells}, "
                f"0.05*free_cells={0.05 * free_cells:.1f} -> max_agents={max_agents}. "
                f"Using fallback agent_count={agent_counts[0]}."
            )
        else:
            agent_counts = []
            print(f"  WARNING [{map_tag}]: no free cells — skipping map entirely.")
    elif max_agents <= 100:
        agent_counts = [c for c in [15, 30, 45, 60, 75, 100] if c <= max_agents]
    elif max_agents <= 200:
        agent_counts = [c for c in [25, 50, 75, 100, 125, 150, 175, 200] if c <= max_agents]
    elif max_agents <= 400:
        agent_counts = [c for c in [50, 100, 150, 200, 250, 300, 350, 400] if c <= max_agents]
    else:
        agent_counts = list(range(100, max_agents + 1, 100))

    if num_humans < 1:
        num_humans = max(1, min(free_cells // 10, 5))
        print(
            f"  WARNING [{map_tag}]: computed num_humans=0, "
            f"using fallback num_humans={num_humans}."
        )
...
# ---------------------------------------------------------------------------
# Simulation helpers (following run_solver_study.py patterns)
# ---------------------------------------------------------------------------
...
def _write_convergence_csv(results: List[Dict[str, Any]], path: Path) -> None:
    ...
    For each (map_tag, human_model_tag, solver_label, num_agents) group, averages
    the throughput_timeline across seeds and writes step-by-step data. This enables
    convergence plots showing that throughput stabilizes well before 2000 steps.
    """
    path.parent.mkdir(parents=True, exist_ok=True)
...
def _write_summary_csv(results: List[Dict[str, Any]], path: Path) -> None:
    """Aggregate results: mean +/- std per (map_tag, human_model_tag, solver_label, num_agents)."""
    path.parent.mkdir(parents=True, exist_ok=True)

    # Group by identifying keys
    group_keys = ("map_tag", "human_model_tag", "solver_label", "num_agents")
...
def _print_results_table(results: List[Dict[str, Any]]) -> None:
    ...
    for mt in map_tags_seen:
        ...
        print(f"Map: {mt}")
        header = (
            f"  {'Solver':<12} {'HumanModel':<10} {'Agents':>6}  {'n':>2}  "
        ...
        entries = sorted(
            ...
            key=lambda kv: (-_mean(kv[1], "throughput"), kv[0][1], kv[0][2], kv[0][3]),
        )
        for (_, hm, solver, n_agents), recs in entries:
            ...
            print(
                f"  {solver:<12} {hm:<10} {n_agents:>6}  {n:>2}  "
...
def run_full_study(
    ...
    """Run the full factorial comparison study."""
    ...
    for mt in map_tags:
        mdef = MAP_DEFS[mt]
        info = analyse_map(mt, mdef["map_path"], safety_radius=mdef.get("safety_radius", 2))
    ...
    for minfo in map_infos:
        mt = minfo["map_tag"]
        # Select solvers for this map (large maps get fewer solvers)
        if mt in LARGE_MAPS:
            map_solvers = [s for s in solver_labels if s in LARGE_MAP_SOLVER_LABELS]
        else:
            map_solvers = [s for s in solver_labels if s in SMALL_MAP_SOLVER_LABELS]
    ...
    # Save metadata
    metadata = {
        "study_name": "compare_global_planners",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Global Planner Comparison Study for HA-LMAPF.\n"
            "Compares 6 global planners across 4 maps and 3 human-model settings."
        ),
    ...
    parser.add_argument(
        "--out", type=str, default="logs/solvers/agents",
        help="Base output directory (default: logs/solvers/agents)",
    )
    parser.add_argument(
        ...
        help=(
            ...
            "Defaults to a timestamp: compare_YYYYMMDD_HHMMSS"
        ),
    )
    parser.add_argument(
    ...
    # Header
    print(f"\nGlobal Planner Comparison Study")
    ...
    print(f"  python scripts/solvers/plot_compare_global_results_agents.py --run-dir {out_dir}")
