# !/usr/bin/env python3
"""
Plot results from the Human-Count Sweep Study.

Reads the CSV files produced by compare_global_study_human.py and generates
publication-quality figures for each map separately.

Required plots per map
----------------------
  throughput vs number of humans
  task_completion vs number of humans
  planning_time vs number of humans
  safety_violations vs number of humans

Each plot shows one subplot per human-model setting, with lines for each
solver. Error bands show 95% confidence intervals across seeds.

Usage
-----
  # Plot from a completed run directory
  python scripts/solvers/plot_compare_global_results_human.py \\
      --run-dir logs/solvers/human/human_sweep_20260315_120000
  ...
  # Specific format
  python scripts/solvers/plot_compare_global_results_human.py \\
      --run-dir logs/solvers/human/human_sweep_20260315_120000 --format pdf
...
SOLVER_ORDER = ["lacam", "lacam3", "pibt2", "lns2", "PBS", "cbsh2-rtc"]
...
def aggregate_raw(
    ...
    Grouping key: (map_tag, human_model_tag, solver_label, num_humans)
    Returns: {key: (mean, ci95)}
    """
...
def plot_metric_for_map(
    ...
    X-axis: number of humans (the sweep variable)
    ...
    # Collect human models, solvers, and human counts present for this map
    hm_tags = set()
    solver_labels = set()
    human_counts = set()
    for (mt, hm, solver, n_humans), _ in data.items():
        if mt == map_tag:
            hm_tags.add(hm)
            solver_labels.add(solver)
            human_counts.add(n_humans)
    ...
    human_list = sorted(human_counts)
    ...
    for ci, hm in enumerate(hm_list):
        ...
        for si, solver in enumerate(solver_list):
            ...
            # Shaded band for visual clarity
            ax.fill_between(
                xs,
...
def plot_convergence(
    ...
    Picks representative configs: mid-range human count per map, first
    ...
    for mt in maps_available:
        human_counts = sorted({r["num_humans"] for r in rows if r["map_tag"] == mt})
        hm_tags = sorted({r["human_model_tag"] for r in rows if r["map_tag"] == mt})
        if human_counts and hm_tags:
            mid_humans = human_counts[len(human_counts) // 2]
            representative[mt] = (hm_tags[0], mid_humans)
    ...
    for mi, (mt, (hm, n_humans)) in enumerate(sorted(representative.items())):
        ax = axes[0][mi]
        solver_set = sorted({r["solver_label"] for r in rows
                             if r["map_tag"] == mt and r["human_model_tag"] == hm
                             and r["num_humans"] == n_humans})
        ...
        for si, solver in enumerate(solver_list):
            ...
            for r in rows:
                if (r["map_tag"] == mt and r["human_model_tag"] == hm
                        and r["solver_label"] == solver and r["num_humans"] == n_humans):
                    steps.append(r["step"])
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to a completed run directory under logs/solvers/human/",
    )
    parser.add_argument(
        "--output", "-o",
        type=str,
